...
def project_velo_to_img(points, T_cam_velo, R_rect, P_rect) -> tuple:
    ...
    idxs = (points_2d_cam[2, :] >= 0)
    points_2d_cam = P_rect.dot(points_3d_cam[:, idxs])

    return points_3d_cam[:, idxs], points_2d_cam / points_2d_cam[2, :], idxs


def align_img_and_velo(img_dir: str, pc_dir: str, calib_dir: str) -> np.ndarray:
    img = imread(img_dir)
    points = load_velo_points(pc_dir)
